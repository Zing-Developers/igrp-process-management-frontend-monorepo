...
export class AreaClient extends BaseApiClient {
     ...
     */
    async updateArea(id: string, area: UpdateAreaRequest): Promise<ApiResponse<Area>> {
     ...
     */
    async getAreaProcesses(areaId: string, params?: {
        processKey?: string;
        status?: string;
        releaseId?: string;
        page?: number;
        ...
        return this.get<PaginatedResponse<Process>>(`/areas/${areaId}/process-definitions`, params);
    }

    /**
     * POST /areas/{areaId}/process-definitions - Associate a process to an area
     */
    async associateProcessToArea(areaId: string, processData: {
        processKey: string;
        releaseId: string;
        version: string;
        name: string;
    }): Promise<ApiResponse<Process>> {
        return this.post<Process>(`/areas/${areaId}/process-definitions`, processData);
    }

    /**
     * DELETE /areas/{areaId}/process-definitions/{processDefinitionId} - Remove a process from an area
     */
    async removeProcessFromArea(areaId: string, processDefinitionId: string): Promise<ApiResponse<void>> {
        return this.delete<void>(`/areas/${areaId}/process-definitions/${processDefinitionId}`);
    }
}