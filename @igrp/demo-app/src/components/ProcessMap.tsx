...
export function ProcessMap() {
  ...
  const buildProcessTree = async () => {
    try {
      ...
      const [areasResponse, projectsResponse] = await Promise.all([
        ...
        getAllProjects(0, 1000) // Get all projects with their process definitions
      ])
  ...
  const buildProjectNode = (project: Project): ProcessTreeNode => {
    // Get processes from the project's processDefinitions
        ...
        return (
          <div className="flex items-center space-x-2">
            ...
            <span className="px-2 py-1 rounded-full text-xs font-medium bg-purple-100 text-purple-800">
              {project.processDefinitions.length} processos
            </span>
          </div>
        )
      }
              ...
              <div className="flex-1 min-w-0">
                ...
                {node.type === 'project' && (
                  <p className="text-xs text-gray-400">
                    Código: {(node.data as Project).code}
                  </p>
                )}
                {node.type === 'process' && (
                  <p className="text-xs text-gray-400">
                    Código: {(node.data as Process).code} | Chave: {(node.data as Process).processKey}
                  </p>
                )}
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="text-gray-900">{selectedProcess.code}</p>
                  </div>
                  <div>
                ...
                {selectedProcess.deploymentDate && (
                  <div>
