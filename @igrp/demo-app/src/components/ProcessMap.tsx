...
export function ProcessMap() {
  ...
  const buildProcessTree = async () => {
    try {
      ...
      // Fetch all data
      const [areasResponse, projectsResponse, processesResponse] = await Promise.all([
        getAreas(0, 100), // Get all areas
        getAllProjects(0, 1000), // Get all projects
        getProcesses(0, 1000) // Get all processes
      ])
      ...
      const tree = buildTree(areas, projects, processes)
      setTreeData(tree)
    } catch (error) {
  ...
  const buildTree = (areas: Area[], projects: Project[], processes: Process[]): ProcessTreeNode[] => {
    ...
    for (const area of rootAreas) {
      const areaNode = buildAreaNode(area, areas, projects, processes)
      tree.push(areaNode)
    }
  ...
  const buildAreaNode = (area: Area, allAreas: Area[], allProjects: Project[], allProcesses: Process[]): ProcessTreeNode => {
    const children: ProcessTreeNode[] = []

    // Get subareas first
    const subareas = allAreas.filter(subarea => subarea.area_fk === area.id)
    for (const subarea of subareas) {
      const subareaNode = buildAreaNode(subarea, allAreas, allProjects, allProcesses)
      children.push(subareaNode)
    }

    // Get projects directly associated with this area
    // Note: In a real implementation, you would fetch AreaProject associations
    // For now, we'll simulate this by checking if projects should be associated with this area
    const areaProjects = getProjectsForArea(area.id, allProjects)
    
    for (const project of areaProjects) {
      const projectNode = buildProjectNode(project, allProcesses)
  ...
  const getProjectsForArea = (areaId: string, allProjects: Project[]): Project[] => {
    ...
    const areaProjectAssociations: AreaProject[] = [
      ...
      { id: 'area_proj_1', area_fk: '1', project_id: 'proj_1', createdAt: '2024-01-01T00:00:00Z' },
      { id: 'area_proj_2', area_fk: '1', project_id: 'proj_2', createdAt: '2024-01-01T00:00:00Z' },
      // Development subarea projects
      { id: 'area_proj_3', area_fk: '3', project_id: 'proj_1', createdAt: '2024-01-01T00:00:00Z' },
      { id: 'area_proj_4', area_fk: '3', project_id: 'proj_3', createdAt: '2024-01-01T00:00:00Z' },
      // Infrastructure subarea projects
      { id: 'area_proj_5', area_fk: '4', project_id: 'proj_2', createdAt: '2024-01-01T00:00:00Z' },
      // HR area projects
      { id: 'area_proj_6', area_fk: '2', project_id: 'proj_4', createdAt: '2024-01-01T00:00:00Z' },
      // Recruitment subarea projects
      { id: 'area_proj_7', area_fk: '5', project_id: 'proj_5', createdAt: '2024-01-01T00:00:00Z' },
      // Payroll subarea projects
      { id: 'area_proj_8', area_fk: '6', project_id: 'proj_4', createdAt: '2024-01-01T00:00:00Z' }
    ]
  ...
  const buildProjectNode = (project: Project, allProcesses: Process[]): ProcessTreeNode => {
    // Get processes for this project
    const projectProcesses = allProcesses.filter(process => process.projectId === project.id)
    
    const children: ProcessTreeNode[] = projectProcesses.map(process => ({
      type: 'process',
      id: process.id,
      name: process.name,
  ...
  const handleStartProcess = async (process: Process) => {
    try {
      ...
      const instance = await startProcess(process.id, `BK_${Date.now()}`, {
        initiatedBy: 'user1',
            ...
            <div className="p-6">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <p className="text-gray-900">{new Date(selectedProcess.deploymentTime).toLocaleString()}</p>
                </div>
              </div>
            </div>
          </div>
