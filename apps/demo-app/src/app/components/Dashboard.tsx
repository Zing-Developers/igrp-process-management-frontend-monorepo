...
export function Dashboard() {
  ...
  const [myTasks, setMyTasks] = useState<PaginatedResponse<Task> | null>(null)
  const [availableTasks, setAvailableTasks] = useState<PaginatedResponse<Task> | null>(null)
  ...
  const stats = [
    ...
    },
    {
      ...
      value: availableTasks?.totalElements || 0,
      icon: Clock,
          ...
          <div className="space-y-3">
            {processes?.content.slice(0, 5).map((process) => (
              <div key={process.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                <div>
                  <p className="font-medium text-gray-900">{process.name || process.processKey}</p>
                ...
                <span className={`px-2 py-1 text-xs font-medium rounded-full ${
                  ...
                  {process.statusDesc || process.status}
                </span>
              </div>
            ))}
          </div>
